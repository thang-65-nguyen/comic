    ...
    const fetchComicsData = async () => {
      try {
        const responses = await Promise.all(
          stories.map((story) =>
            fetch(`https://otruyenapi.com/v1/api/truyen-tranh/${story.slug}`)
          )
        );
      ...
      >
        {archives.length === 0 ? (
          <span className="text-2xl">
            <i className="fa-regular fa-bookmark"></i> Kho lưu trữ trống!
          </span>
        ) : (
          <>
            <span className="text-2xl">
              <i className="fa-regular fa-bookmark"></i> Kho lưu trữ (
              {archives.length})
            </span>
            <div
